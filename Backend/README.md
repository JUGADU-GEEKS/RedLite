...
## PRR-MASC Lane Switching Subsystem

This system implements a Priority-Based Round Robin (PRR) algorithm with Multi-Agent System Control (MASC) concepts.
Reference: `/mnt/data/2109.00937v2.pdf`

### Features
- **Density-Based Ranking**: Lanes are ranked by vehicle density (descending).
- **Age Tie-Breaking**: If densities are equal, the lane with the higher "age" (wait time score) wins.
- **Fixed Durations**: Top ranked lanes get longer green times: [45s, 30s, 15s, 15s].
- **Robust Detection**: Uses YOLOv8 by default. If model loading fails, automatically falls back to OpenCV MOG2 background subtraction.
- **Real-Time Updates**: WebSocket `/ws/lane_feed` broadcasts per-second updates including base64 frames, counts, and phase info.
- **IoT Polling**: IoT devices can poll `/lane/signal_state` to get the current light status.

### Configuration
- `MODEL_PATH`: Path to YOLO model (default `yolov8n.pt`).
- `VIDEOS_DIR`: Directory containing simulation videos (`1.mp4`...`4.mp4`).
- `MONGO_URL`: MongoDB connection string. Fallback to JSON files in `Backend/data/` if not provided.

### Endpoints

- `GET /lane/signal_state?intersectionId=...` (Public/IoT)
  - Returns current signal state (red/green/yellow status, remaining time).
  - **cURL**:
    ```bash
    curl "http://localhost:8000/lane/signal_state?intersectionId=INT-001"
    ```

- `POST /lane/run_cycle?intersectionId=...` (Employee/Admin)
  - Manually triggers a cycle plan calculation.
  - **cURL**:
    ```bash
    curl -X POST "http://localhost:8000/lane/run_cycle?intersectionId=INT-001" -H "Authorization: Bearer <TOKEN>"
    ```

- `WebSocket /ws/lane_feed?intersectionId=...`
  - Connect to receive real-time updates.
  - Payload includes: `phase`, `lane`, `remaining`, `counts`, `frames` (base64), `priority_order`, `durations`.

### Running with YOLO vs Fallback
- **YOLO**: Ensure `ultralytics` is installed and `yolov8n.pt` is present in `Backend/`.
- **Fallback**: If `ultralytics` is missing or model fails to load, the system logs a warning and uses OpenCV background subtraction. This ensures the system never crashes due to AI model issues.
